...
extension MenuBarItemManager {
    ...
    private nonisolated func getEventSource(
        ...
        source.userData = MouseHelpers.syntheticEventSignature
        MenuBarItemManager.diagLog.debug("CURSORTRACE tagged event source state=\(stateID.rawValue) userData=\(source.userData)")
...
extension MenuBarItemManager {
    ...
    func move(
        ...
        // Movement during a move can be a consequence of our own events.
        // Keep this observer only as a conservative reason to stop retries,
        // never as a source of restore coordinates. MouseHelpers alone owns
        // the trusted target and rejects samples throughout active scopes.
        let cursorMovementObserved = OSAllocatedUnfairLock(initialState: false)
        let cursorWatch = EventMonitor.universal(for: .mouseMoved) { event in
            guard let cgEvent = event.cgEvent else { return event }
            if MouseHelpers.isSyntheticCursorEvent(cgEvent) {
                MenuBarItemManager.diagLog.debug("CURSORTRACE retry observer ignored our synthetic movement")
                return event
            }
            cursorMovementObserved.withLock { $0 = true }
            return event
        }
        cursorWatch.start()
        ...
        for n in 1 ... maxAttempts {
            ...
            // Unclassified movement can still be real input. Avoid another
            // attempt after observing it, but never trust its coordinates.
            // Untagged movement generated by the window server can also
            // stop retries here; it cannot overwrite the restore target.
            if n > 1, cursorMovementObserved.withLock({ $0 }) {
                MenuBarItemManager.diagLog.debug(
                    "Giving up remaining move attempts for \(item.logString): cursor movement was observed"
                )
                throw EventError.cannotComplete
            }
            do {
...
private extension CGEvent {
    ...
    func post(to location: EventTap.Location) {
        let type = self.type
        MenuBarItemManager.diagLog.debug("CURSORTRACE post type=\(type.logString) userData=\(getIntegerValueField(.eventSourceUserData))")
        MenuBarItemManager.diagLog.debug(
            """
    ...
    private func setUserData() {
        // Keep barrier identities unique while marking every event as ours.
        let userData = MouseHelpers.nextSyntheticEventUserData()
